...
//        checkOpen(9600);
    ...
    public event EventHandler<SerialMessageEventArgs> SerialDataReceived;
    ...
    protected void receivedData(string data)
    {
        if (NotifyData)
        {
            if (SerialDataReceived != null) SerialDataReceived(this, new SerialMessageEventArgs(data));
        }
        ...
        if (NotifyLines || NotifyValues)
        {
            ...
            for (int iLine = 0; iLine < nLines - 1; iLine++)
            {
                ...
                if (RememberLines > 0)
                {
                    ...
                    if (overflow > 0)
                    {
                        print("Serial removing " + overflow +
                              " lines from lines buffer. Either consume lines before they are lost or set RememberLines to 0.");
    ...
    {
        return AppManager.Instance.Settings["COM Port"].StringValue;
